...
import { resetPromptGate } from './canProtocol.js';
...
export async function reinitAdapter(reason, log = null) {
    ...
    state.reinitInProgress = true;   // черга запитів мовчить, поки йде ATZ+init
    ...
    try {
        ...
        // ATZ обовʼязковий. Перша версія цього відновлення слала лише ATE0 і була
        // зареверчена саме тому, що адаптер його ігнорував і продовжував ехоїти:
        // після ERR9x клон сидить у збійному стані, з якого його виводить тільки
        // повний ресет. Кулдаун нижче не дає цьому крутитись у циклі.
        await runElmInit({ reset: true, log });
        resetPromptGate();
